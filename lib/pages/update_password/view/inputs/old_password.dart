...
class OldPasswordField extends StatefulWidget {
  ...
  const OldPasswordField({Key? key, this.controller}) : super(key: key);

  @override
  OldPasswordFieldState createState() => OldPasswordFieldState();
}

class OldPasswordFieldState extends State<OldPasswordField> {
  ...
  String? getError(Password data, TranslateType translate) {
    if (data.invalid) {
      if (data.error == PasswordValidationError.empty) {
    ...
    return BlocBuilder<UpdatePasswordBloc, UpdatePasswordState>(
      ...
      builder: (context, state) {
        return InputTextField(
          label: translate('inputs:text_old_password'),
          isRequired: true,
          controller: widget.controller,
          obscureText: _obscureText,
          onChanged: (value) {
            context.read<UpdatePasswordBloc>().add(OldPasswordChanged(value));
          },
          decoration: InputDecoration(
            errorText: getError(state.oldPassword, translate),
            prefixIcon: Icon(
