...
class OrderChartCubit extends Cubit<OrderChartState> {
  ...
  Future<void> getChart({bool isAdmin = false, FilterChartOption? option, DateTime? start, DateTime? end}) async {
    if (option != null) {
      emit(state.copyWith(
    ...
    if(isAdmin){
      await _getChartWoo(start: start, end: end);
    }else{
  ...
  Future<void> _getChartWoo({DateTime? start, DateTime? end}) async {
    var param = <String, dynamic>{
    ...
    switch (state.filterChartOption) {
      case FilterChartOption.year:
        param.addAll({"period": "year"});
        break;
      case FilterChartOption.lastMonth:
        param.addAll({"period": "last_month"});
        break;
      case FilterChartOption.sevenDays:
        param.addAll({"period": "week"});
        break;
      case FilterChartOption.custom:
        param.addAll({"date_min": _formatDateToParam(start), "date_max": _formatDateToParam(end)});
        break;
      default:
        param.addAll({"period": "month"});
    }
    try {
      final Map<String, dynamic> data = await salesByDateRepository.getChartByDateWoo(
        requestData: RequestData(cancelToken: _cancelToken, query: param),
      );
      OrderChartModel chart = data['chart_model'];
      LegendData legendData = data['legend_data'];
      String? currency = data['currency'];
