...
class ProductAddState extends Equatable {
  const ProductAddState({
    ...
    this.addOnsTemporaryStorage = const [],
    this.addOnsData = const {},
    this.saveAddOnsState = const InitState(),
  });
  ...
  /// Just for store add-ons data
  final List<ProductAddOnModel> addOnsTemporaryStorage;

  final BaseState saveAddOnsState;

  final Map<dynamic, dynamic> addOnsData;
  ...
  ProductAddState copyWith({
    ...
    List<ProductAddOnModel>? addOnsTemporaryStorage,
    Map<dynamic, dynamic>? addOnsData,
    BaseState? saveAddOnsState,
  }) {
    return ProductAddState(
      ...
      addOnsTemporaryStorage: addOnsTemporaryStorage ?? this.addOnsTemporaryStorage,
      addOnsData: addOnsData ?? this.addOnsData,
      saveAddOnsState: saveAddOnsState ?? this.saveAddOnsState,
    );
  }
...
ProductAddState _productFromJson(Map<String, dynamic> json) {
  ...
  List? metaData = json['meta_data'] ?? [];
  dynamic addOnsData = metaData?.firstWhereOrNull((element) => element['key'] == '_product_addons') ?? <dynamic, dynamic>{'key': '_product_addons'};
  var valueAddOns = addOnsData['value'];
  List<ProductAddOnModel> addOns = [];
  if(valueAddOns is List){
    for (var element in valueAddOns) {
      addOns.add(ProductAddOnModel.fromJson(element));
    }
  }else if(valueAddOns is Map){
    valueAddOns.forEach((key, value) {
      addOns.add(ProductAddOnModel.fromJson(value));
    });
  }
...
Map<String, dynamic> _productToJson(ProductAddState instance) {
  Map addOnsData = instance.addOnsData;
  List addOns = instance.productAddOns.map((e) => e.toJson()).toList();
  addOnsData['value'] = addOns;
