...
class ProductAddBloc extends Bloc<ProductAddEvent, ProductAddState> {
  ...
  ProductAddBloc({
    ...
    on<AddFieldAddOns>(_addFieldAddOns);

    on<RemoveFieldAddOnsAt>(_removeAddOns);
    on<UpdateAddOnsStorage>(_updateAddOnsStorageAt);
  ...
  /// Add new add-ons field.
  void _addFieldAddOns(AddFieldAddOns event, Emitter<ProductAddState> emit) {
    // Add-ons in addOnsTemporaryStorage is newest add-on.
    List<ProductAddOnModel> addons = [...state.addOnsTemporaryStorage];
    addons.add(const ProductAddOnModel());
    emit(state.copyWith(
      productAddOns: addons,
      addOnsTemporaryStorage: addons,
    ));
  }

  /// Remove add-ons
  void _removeAddOns(RemoveFieldAddOnsAt event, Emitter<ProductAddState> emit) {
    // Add-ons in addOnsTemporaryStorage is newest add-on.
    List<ProductAddOnModel> addons = [...state.addOnsTemporaryStorage];
  ...
  bool _validateAddOns() {
    ...
    for (var addOns in state.productAddOns) {
      if (addOns.name == null || addOns.name?.trim().isEmpty == true) {
        validData = false;
        break;
      }
      if (addOns.options == null) {
        validData = false;
        break;
      }
      if ((addOns.type == AddOnsType.multiChoice.getKey() || addOns.type == AddOnsType.checkBox.getKey()) &&
          addOns.options != null) {
        for (var option in addOns.options!) {
          if (option.label?.trim().isEmpty == true) {
            validData = false;
            break;
          }
        }
  ...
  Future<void> _saveAddOns(
    ...
    emit(state.copyWith(
      productAddOns: state.addOnsTemporaryStorage,
      saveAddOnsState: const LoadingState(),
    ));

    if (!_validateAddOns()) {
      emit(
        state.copyWith(
          saveAddOnsState: const ErrorState(
              data: 'validate:text_invalid_add_on'),
        ),
      );
      return;
    }
    ...
    try {
      Map addOnsData = state.addOnsData;
      addOnsData.remove('value');
      addOnsData.addAll(<String, dynamic>{'value': state.productAddOns.map((e) => e.toJson()).toList()});
      Map<String, dynamic> data = {
        'meta_data': [addOnsData]
      };
      ...
      List<ProductAddOnModel> addOnsNew = [];
      dynamic addOnsDataNew;
      if (metaData != null) {
        addOnsDataNew = metaData.firstWhereOrNull((element) => element['key'] == '_product_addons') ?? <dynamic, dynamic>{'key': '_product_addons'};
        var valueAddOns = addOnsData['value'];
        if (valueAddOns is List) {
          for (var element in valueAddOns) {
            addOnsNew.add(ProductAddOnModel.fromJson(element));
          }
        } else if (valueAddOns is Map) {
          valueAddOns.forEach((key, value) {
            addOnsNew.add(ProductAddOnModel.fromJson(value));
          });
        }
      }
      emit(state.copyWith(
        productAddOns: addOnsNew,
        addOnsData: addOnsDataNew,
        saveAddOnsState: const LoadedState(),
      ));
    } on RequestError catch (e) {
      emit(state.copyWith(
        saveAddOnsState: ErrorState(data: e.message.isNotEmpty ? e.message : 'Error'),
      ));
    }
  }

  void _updateAddOnsStorageAt(UpdateAddOnsStorage event, Emitter<ProductAddState> emit) {
    if (event.index != null && event.productAddOnModel != null) {
      List<ProductAddOnModel> addons = [...state.addOnsTemporaryStorage];
      addons.replaceRange(event.index!, (event.index! + 1), [event.productAddOnModel!]);
      emit(state.copyWith(addOnsTemporaryStorage: addons));
    } else if (event.listProductAddOnModel != null) {
      List<ProductAddOnModel> addons = [...event.listProductAddOnModel!];
      emit(state.copyWith(addOnsTemporaryStorage: addons));
    }
  }
  ...
  void _onSubmitted(ProductAddSubmitted event, Emitter<ProductAddState> emit) async {
    ...
    if (!_validateAddOns()) {
      ...
      emit(state.copyWith(
        saveAddOnsState: const InitState(),
      ));
    }
