    ...
    return BlocConsumer<OrderDetailCubit, OrderDetailState>(
      builder: (context, state) {
        ...
        return Scaffold(
          ...
          body: Stack(children: [
            child,
            if (state.buttonStatus is LoadingState) Center(child: buildLoading()),
          ]),
        );
      },
      listenWhen: (previous, current) => previous.buttonStatus != current.buttonStatus,
      listener: (context, state) {
        if (state.updateOrderStatus == UpdateOrderStatus.failure) {
          if (state.statusError != '') {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text(state.statusError)),
            );
          } else {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text(translate('order:text_save_order_error'))),
            );
          }
        } else if (state.statusNew) {
        ...
        customerNote: order?.customerNote != ''
            ? RichText(
                text: TextSpan(
                  ...
                  children: <TextSpan>[
                    TextSpan(text: '${order?.customerNote}', style: textTheme.bodySmall),
                  ],
                ),
              )
            : null,
  ...
  _buildButtonChangeStatus(TranslateType translate, int orderId) {
    return Center(
      child: BlocBuilder<OrderDetailCubit, OrderDetailState>(
        buildWhen: (previous, current) => (previous.buttonStatus != current.buttonStatus),
        builder: (context, state) {
          return SizedBox(
            width: 161,
            child: ElevatedButton(
              onPressed: () {
                if (state.buttonStatus is! LoadingState) {
                  cubit.updateOrderStatus(context, orderId);
                }
              },
              style: ElevatedButton.styleFrom(padding: EdgeInsets.zero),
              child: state.buttonStatus is LoadingState
