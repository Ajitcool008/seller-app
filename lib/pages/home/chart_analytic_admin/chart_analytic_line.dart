...
class ChartAnalyticAdminLine extends StatelessWidget {
  final List<AnalyticData> data;

  const ChartAnalyticAdminLine({
    Key? key,
    required this.data,
  }) : super(key: key);
        ...
        LineChartData(
                ...
                List<LineTooltipItem> lineBarSpot = touchedSpots.map(
                  (e) {
                    ...
                    return LineTooltipItem(
                      '$count',
                      (theme.textTheme.bodySmall ?? const TextStyle()).copyWith(color: Colors.white),
                    );
                  },
                ).toList();
          ...
          titlesData: FlTitlesData(
            ...
            bottomTitles: SideTitles(
              ...
              reservedSize: 30,
              interval: 1,
              getTextStyles: (context, value) => theme.textTheme.labelSmall,
              getTitles: (value) {
                int visit = value.toInt();
                if (visit < data.length) {
                  return formatDateToDayOfWeek(
                    date: data[visit].name,
                    format: 'yyyy-MM-dd',
                  );
                }
                return '...';
            ...
            leftTitles: SideTitles(
              ...
              getTextStyles: (context, value) =>
                  theme.textTheme.labelSmall?.copyWith(color: theme.textTheme.bodySmall?.color),
              getTitles: (value) {
                if (spots.isEmpty) {
                  return '';
                }
