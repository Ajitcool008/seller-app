...
class SaleProductRepository {
  ...
  ///
  /// Throws an [Error] if client not get data. Returns list data sale product.
  Future<List<SaleProduct>> getBestSellerProductsWoo({required RequestData requestData}) async {
    try {
      dynamic res = await _httpClient.get(
        Endpoints.getBestSellerProducts,
        data: requestData,
      );
      List<SaleProduct> data = List<SaleProduct>.of([]);
      if (res is List) {
        int topNumber = 5;
        if (topNumber > res.length) topNumber = res.length;
        for (int index = 0; index < topNumber; index++) {
          data.add(SaleProduct(
            count: res[index]?['quantity'],
            name: res[index]?['name'],
          ));
        }
      }
