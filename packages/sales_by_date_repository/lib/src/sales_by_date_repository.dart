...
class SalesByDateRepository {
  ...
  /// **Get chart sales by date Woocomerce API**
  ///
  /// **Param:**
  ///
  /// requestData: your request data
  ...
  /// currency: data['currency'], priceDecimal: data['price_decimal'],
  /// legendData: data['legend_data')
  Future<Map<String, dynamic>> getChartByDateWoo({required RequestData requestData}) async {
    try {
      var res = await _httpClient.get(
        Endpoints.getSalesReportWoo,
        data: requestData,
      );
      Map<String, dynamic> result = {};
      OrderChartModel chart = OrderChartModel.fromJsonWoo(res?[0] ?? {});
      LegendData legendData = LegendData.fromJsonWoo(res?[0] ?? {});
      result.addAll({
        "chart_model": chart,
        "legend_data": legendData
      });
